...
pub async fn login_controller(login_data: Json<LoginData>) -> impl Responder {
    ...
    match User::find_by_email(&login_data.email).await {
        Some(user) => match user.verify_password(&login_data.password).await {
            true => {
                ...
                match claim.jwt_token() {
                    Some(jwt) => {
                        ...
                        AccessTokenClaims::new(&user.id.to_string())
                            .await
                            .set()
                            .await;
                        RefreshTokenClaims::new(&user.id.to_string())
                            .await
                            .set()
                            .await;
                        info!(
                            "Time taken to generate new token {}",
            ...
            false => {
                ...
                info!(
                    "User with email id {} failed logged in successfully",
                    user.email.clone()
                );
        ...
        _ => {
            info!("Time to err {}", time.elapsed().as_millis());
            info!(
                "User with email id {} not found failed to login",
