...
impl PendingMessage {
    pub async fn send(dst_uids: Vec<String>, message: &MessageEvent) -> RedisResult<()>{
        ...
        if message.dst_uids.is_empty() { 
            return Err(RedisError::from((ErrorKind::ParseError, "Message destination is null")));
        }
        for dst in dst_uids {
    ...
    pub async fn fetch(uid: String) -> RedisResult<Vec<MessageEvent>> {
        let serialized_messages: Vec<String> = get_redis_conn().await.as_mut().unwrap()
                    .lrange(format!("pending_messages:{}", uid ), 0, -1)?;
        let msgs = serialized_messages.clone();
        let id = uid.clone();
        let message = serialized_messages.into_iter()
            .filter_map(|m|
