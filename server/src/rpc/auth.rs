...
use crate::types::{
    auth, AuthRequest, AuthToken, CreateRoleResponse, Credentials, OAuth, RefreshToken,
    RegisterStatus, Revoke, Token,
};
use crate::types::{RoleRequest, RoleResponse};
...
lazy_static! {
    static ref jwt_secret_key: Arc<String> =
        Arc::new(std::env::var("JWT_KEY").unwrap_or("dev".to_string()));
    static ref jwt_secret_refresh_key: Arc<String> =
        Arc::new(std::env::var("JWT_REFRESH_KEY").unwrap_or("dev".to_string()));
    static ref access_token_ttl: Arc<String> =
        Arc::new(std::env::var("JWT_ACCESS_TOKEN_TTL").unwrap_or("1".to_string()));
    static ref refresh_token_ttl: Arc<String> =
        Arc::new(std::env::var("JWT_TOKEN_TOKEN_TTL").unwrap_or("720".to_string()));
}
...
impl Auth for AuthServerImpl {
    async fn auth(&self, request: Request<AuthRequest>) -> Result<Response<Token>, Status> {
        ...
        if let Some(data) = auth_type.data {
            return match data {
                Data::Credentials(credentials) => self.token_credentials(credentials).await,
                Data::Oauth(oauth) => self.token_oauth(oauth).await,
            };
        }
    ...
    async fn token(&self, request: Request<RefreshToken>) -> Result<Response<AuthToken>, Status> {
        ...
        if let Ok(claims) = RefreshTokenClaims::from_jwt(refresh_token.token) {
            ...
            let token = AccessTokenClaims::new(&claims.sub)
                .await
                .jwt_token()
                .unwrap();
            return Ok(Response::new(AuthToken {
    ...
    ) -> Result<Response<RegisterStatus>, Status> {
        ...
        match result {
            Ok(user) => {
                if let Err(e) = user.insert_new_user().await {
                    return Ok(Response::new(RegisterStatus { status: e.into() }));
                }
    ...
    async fn role(&self, request: Request<RoleRequest>) -> Result<Response<RoleResponse>, Status> {
        match Roles::get_role(request.into_inner().email).await {
            None => Ok(Response::new(RoleResponse {
                role: "NONE".to_string(),
            })),
            Some(role) => Ok(Response::new(RoleResponse {
                role: role.to_value(),
            })),
        }
    }
    ...
    async fn create_role(
        &self,
        request: Request<auth::CreateRoleRequest>,
    ) -> Result<Response<CreateRoleResponse>, Status> {
        let role = request.into_inner();
        match Roles::insert(role.email, Usertype::from(role.role)).await {
            Ok(_) => Ok(Response::new(CreateRoleResponse {
                status: auth::Status::Ok.into(),
            })),
            Err(_) => Ok(Response::new(CreateRoleResponse {
                status: auth::Status::InternalError.into(),
            })),
        }
    }
}
...
impl AuthServerImpl {
        ...
        match User::find_by_email(&credentials.email).await {
            Some(user) => {
                if !user.verify_password(&credentials.password).await {
                    return Self::error(
                        types::Status::InvalidCredentials,
    ...
    async fn token_from(&self, user: User) -> Result<Response<Token>, Status> {
        let attl = access_token_ttl.parse().expect("Invalid access token ttl");
        let rtll = refresh_token_ttl
            .parse()
            .expect("Invalid refresh token ttl");
        if let Some((access_token, refresh_token)) =
            AuthServerImpl::get_cached_tokens(&user.id.to_string()).await
        {
            let access_token_expiration = Some(prost_types::Timestamp::from(
                SystemTime::now().add(std::time::Duration::from_secs(
                    access_token.exp as u64 - access_token.iat as u64,
                )),
            ));
            let refresh_token_expiration = Some(prost_types::Timestamp::from(
                SystemTime::now().add(std::time::Duration::from_secs(
                    refresh_token.exp as u64 - refresh_token.iat as u64,
                )),
            ));
            Ok(Response::new(Token {
            ...
            let (access_token, refresh_token) = (
                access_claim.jwt_token().unwrap(),
                refresh_claim.jwt_token().unwrap(),
            );
            let current_time = SystemTime::now();
            let access_token_expiration = prost_types::Timestamp::from(
                current_time.add(std::time::Duration::from_hours(attl)),
            );
            let refresh_token_expiration = prost_types::Timestamp::from(
                current_time.add(std::time::Duration::from_hours(rtll)),
            );
            Ok(Response::new(Token {
                uid: user.id.to_string(),
                email: user.email,
    ...
    async fn get_cached_tokens(uid: &str) -> Option<(AccessTokenClaims, RefreshTokenClaims)> {
        ...
        if access_token.is_none() || refresh_token.is_none() {
            ...
            let access_token: AccessTokenClaims =
                serde_json::from_str(&access_token.unwrap()).expect("Failed to parse access token");
            let refresh_token: RefreshTokenClaims = serde_json::from_str(&refresh_token.unwrap())
